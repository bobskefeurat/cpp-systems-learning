# Prepare for Gate {{N}}: {{Display Name}}

## Before You Start
You do not need full mastery before you begin.
You only need a workable starting understanding for this gate.

## You Need To Explain
Before you start the task, make sure you can explain these ideas in simple words:

1. {{Core concept 1}}
   - {{subpoint 1}}
   - {{subpoint 2}}
   - {{subpoint 3}}

2. {{Core concept 2}}
   - {{subpoint 1}}
   - {{subpoint 2}}
   - {{subpoint 3}}

3. {{Core concept 3}}
   - {{subpoint 1}}
   - {{subpoint 2}}
   - {{subpoint 3}}

## Read This
Read in this order:

- Start with [{{Primary Resource Card}}](../../../00_system/resources/resource_cards/{{Phase Resource Card Path}}).
  Use the gate-specific packet in that card as your main reading path.
- Use [{{Secondary Resource Card}}](../../../00_system/resources/resource_cards/{{Secondary Phase Resource Card Path}}) only if {{why it is needed}}.
- If the agent later sends you to another curated resource, follow that exact reading assignment.

## Stop When
Move to the readiness check when you can explain:
- {{stop condition 1}}
- {{stop condition 2}}
- {{stop condition 3}}

## Next Step
Complete the readiness dialogue with the agent.
